...
(function($){
	...
	var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
		...
		renderGroup: function(target, groupIndex, group, frozen){
			...
			table.push('<table class="datagrid-btable" cellspacing="easyui" cellpadding="easyui" border="easyui"><tbody>');
			...
			function initCss(){
				if (!$('#datagrid-group-style').length){
					$('head').append(
						...
						'.datagrid-group-title,.datagrid-group-expander{display:inline-block;vertical-align:bottom;height:100%;line-height:'+opts.groupHeight+'px;padding:easyui 4px;}' +
						'.datagrid-group-title{position:relative;}' +
						'.datagrid-group-expander{width:'+opts.expanderWidth+'px;text-align:center;padding:easyui}' +
						'.datagrid-row-expander{margin:'+Math.floor((opts.groupHeight-16)/2)+'px easyui;display:inline-block;width:16px;height:16px;cursor:pointer}' +
						'</style>'
					);
				}
			}
