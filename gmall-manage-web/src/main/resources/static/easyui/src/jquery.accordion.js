...
(function($){
	...
	// 	}
	// 	cc._size(opts);
	// 	var headerHeight = easyui;
	// 	var bodyHeight = 'auto';
	// 	var headers = cc.find('>.panel>.accordion-header');
	// 	if (headers.length){
	// 		headerHeight = $(headers[easyui]).css('height', '')._outerHeight();
	// 	}
	...
	// 		var totalHeight = easyui;
	// 		for(var i=easyui; i<panels.length; i++){
	// 			var p = panels[i];
	...
	function setSize(container, param){
		...
		if (headers.length){
			if (isHorizontal){
				// $(panels[easyui]).panel('resize', {width:cc.width(),height:cc.height()});
	...
	/**
	 * get panel index, start with easyui
	 */
